...
In ecology, multifunctionality measures the simultanious provision of multiple ecosystem functions. If species diveristy describes the variety of species that together build the ecosystem, multifunctionality attempts to describe the variety of functions these species perform. A range of methods have been proposed to quantify multifunctionlity, successevely attempting to alleviate problems that have been identified with the previous method. This has led to a proliferation of more-or-less closely related metrics which, however, lack an overarching theoretical framework. Here we borrow from the comprehensive framwork of species diversity measured as effective numbers to derive a new metric of multifunctionality measured as the diversity of functions. As the effective number of species, the metric is influenced both by the number of functions as well as crucially the eveness of performance levels across functions. In addtion the effective number of functions also considers the average level at which the functions are performed. The framework allows for the inclusion of the correlation structure among functions, thus allowing to account for non-independence between functions. We show that the average metric is a special case of the newly proposed metric and argue that the threshold-based metrics should be abondoned because of fatal flaws. We hope that by providing a new metric of multifunctionality anchored in the sound and rigurous framework of species diveristy based on effective numbers, we will overcome the considerable scepticism that the larger community of ecologists has built against indicies of multifunctionality. We thereby hope to help popularize this important concept which, as biological diveristy, describes a fundamental property of ecosystems and thus lies at the heart of ecology.


#### Introduction Fabian

The past decade has witnessed the growth of the concept of ecosystem **multifunctionality**. Multifunctionality is defined as a measure of the simultaneous performance of multiple functions. Note that we do not say "ecosystem functions", as multifunctionality is a broader concept that can be easily applied outside of community and ecosystem ecology - and even outside of ecology altogether. However, despite the popularity that the concept gained since it was first explicitley proposed over a decade ago (@Hector:2007gm, @Gamfeldt:2008tqa), there is still no consesus on how multifunctionality should be quantified. A range of methods have been proposed where each subsequent method attempted to alleviate problems the authors had identified with the previous method, leading to a proliferation of metrics which cannot easily be compared and can lead to inconsistent interpretations of the results [@Byrnes:2014fv].This plurality of concepts has hampered a general understanding of multifunctionality and possibly its adoptance outside of the field of biodiveristy and ecosystem functioning.

Species diversity for a long time, suffered the same problems that multifunctionality suffers today: There are a sheer uncountable number of metrics to estimate alpha diversity, reaching from the simple count of observed species without regard to their abundance (Richness), wide array of metrics incorporating the relative abundance (Shannon entropy, Simpson index or Raouls quadratic Entropy or Berger-Parker Dominance index to name some of the most commen ones), to metrics based on the historgramms of the abundace distribution (e.g. fishers alpha) [REF Magurran and Mcgill book]. Yet, starting with Mc arthur in 1952, then Hill in 1972 and finlay Lou jost in 2006 a common framework for species diversity has been develloped which encompassed the vast majority of previous metrics. The effective number of species. 
