...
get_mfn_floor <- function(a, k, q){
  ...
  if(q==1){
    ...
    ret <- ak*(ak-g)^(g/ak - 1 )
    ## not sure of the rationale here so leaving it to @jebyrnes to fix
    # ix <- is.nan(ret) 
...
#n_range$n_lower[!is.finite(n_range$n_lower)] <-  0
...
#  geom_ribbon(alpha=0.7) +
  ...
  guides(fill="none", color=guide_legend("Order (q)")) +
  theme_bw(base_size = 14)


##  was getting NaNs when q > 1 and a = 0. I think 
## these should just be zero as A = 0 => functioning is 0(modified above)
```
