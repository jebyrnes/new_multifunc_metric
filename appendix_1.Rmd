...
When $AK-G = 0$, this reduces to 0.
<!-- $AK ^ {\frac{G}{AK}}$, rather than having to divide by 0. -->
RBcomment: this seems like an odd result. 
...
get_mfn_floor <- function(a, k, q){
  ...
  if(q==1){
    ...
    ret <- ak*(ak-g)^(g/ak - 1 )
    ## not sure of the ratioinale here so leaving it to @jebyrnes to fix
    ix <- is.nan(ret) 
...
## something odd going on in the q = 1 case, I think because 
## the equation evaluates to 0 when AK  = G. Is something off here?
## also is it worth just plotting the lower bound and ignoring the upper bound?
## it might make for a neater figure.
```
